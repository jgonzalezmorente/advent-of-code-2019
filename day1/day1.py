...
delivering presents to other planets! To accurately calculate
his position in space, safely align his warp drive, and return 
to Earth in time to save Christmas, he needs you to bring him 
...
Your puzzle answer was 3234871.
...
fuel (654 / 3 - 2). 216 then requires 70 more fuel, which requires 21 fuel, which requires 5 fuel, 
which requires no further fuel. So, the total fuel required for a module of mass 1969 is 654 + 216 + 70 + 21 + 5 = 966.
...
"""
#%%
path = "d:\\advent-of-code-2019\\day1\\"
with open(path+"input.txt") as input:
    modules = [int(m) for m in input]

result = sum(map(lambda m: (m // 3) - 2, modules))

# %%
def calcular_combustible_real(masa, acumulador=0):
    if masa <= 0:
        return acumulador
    else:
        incremento = max((masa // 3) - 2, 0)        
        return calcular_combustible_real(incremento, incremento + acumulador)

result2 = sum(map(calcular_combustible_real, modules))
