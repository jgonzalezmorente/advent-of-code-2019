...
ran Intcode programs like the gravity assist program it was working on; surely there are enough 
...
opcode - either 1, 2, or 99. The opcode indicates what to do; for example, 99 means that the 
...
at positions 10 and 20, add those values, and then overwrite the value at position 30 with 
their sum.
...
of the output (3). To handle this opcode, you first need to get the values at the 
input positions: position 9 contains 30, and position 10 contains 40. Add these numbers 
together to get 70. Then, store this value at the output position; here, the output 
...
Your puzzle answer was 3516593.

The first half of this puzzle is complete! It provides one gold star: *

--- Part Two ---
"Good, the new computer seems to be working correctly! Keep it nearby during this 
mission - you'll probably use it again. Real Intcode computers support many more features 
than your new one, but we'll let you know what they are as you need them."
...
the instruction; until you add more instructions to the computer, this is always 4 (1 opcode + 3 
...
#%%
def ejecutar_programa(input, index=0):
    try:
        if input[index] == 99:
            return input
        elif input[index] == 1:
        ...
        input[input[index+3]] = operacion(input[input[index+1]], input[input[index+2]])
        return ejecutar_programa(input, index=index+4)
    except:
        raise ValueError("El programa introducido como entrada no es válido")
...
# %%
path = "d:\\advent-of-code-2019\\day2\\"
